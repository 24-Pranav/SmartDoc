...
class _StudentUploadTabState extends State<StudentUploadTab> {
  ...
  final AIService _aiService = AIService(apiKey: 'AIzaSyApNjTjqiKjj8qLwJT0zcW9dJAIBoXkewg'); // Replace with your actual API key
  ...
  Future<void> _uploadAndProcessFile() async {
    ...
    try {
      // The redundant authentication calls have been removed.
      // The Supabase client is already authenticated from the initial app login.
      ...
      try {
        if (fileExtension == 'jpg' || fileExtension == 'jpeg' || fileExtension == 'png') {
          ...
          if (aiStatus == 'approved') {
            ...
            status = 'pending';
            comments = aiResponse['comments'] as String? ?? 'AI could not auto-verify. Sent for faculty review.';
          }
        } else if (fileExtension == 'pdf') {
          status = 'pending';
          comments = 'PDF document requires manual faculty review.';
        } else {
          throw Exception('Unsupported file type: $fileExtension');
        }
      } catch (e) {
        status = 'pending';
        comments = 'Verification failed. The document requires faculty review. Error: ${e.toString()}';
      }

      // Reusing the documentId will overwrite the old file in Supabase if it exists.
      final downloadUrl = await _supabaseService.uploadFile(_selectedFile!, documentId, user.id);

      // .set() will create a new doc or completely replace an existing one.
      ...
      String message;
      if (isUpdating) {
        message = 'Document for \'$_selectedCategory\' has been updated.';
      } else {
        message = (status == 'approved')
            ? 'Document automatically approved!'
            : 'Document sent for faculty verification.';
      }
